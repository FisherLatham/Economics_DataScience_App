...
# Grab Google Trends data for Data Science and Economics
...
df <- df %>%
  ...
  summarize(Economics = sum(as.numeric(Economics)),
...
# Develop Shiny App
ui <- fluidPage(theme = shinytheme("united"), # Main theme
                navbarPage(
                  "Economics vs. Data Science:", # Main Title
                  useShinyalert(force = TRUE),
                  tabPanel("Exploring Data", # First tab
                           fluidRow( # Separate into two columns
                             column(6,
                                    h2("Economics"),
                                    plotlyOutput("econts"), # Display general plot
                                    h3("Initial Thoughts"),
                                    textOutput("econtxt"), # Display initial analysis
                                    h3(""),
                                    div(
                                      align = "left",
                                      selectInput(
                                    ...
                                    h3("Analysis"),
                                    textOutput(outputId = "chosetxt"), # Show corresponding analysis
                                    h3("")
                             ),
                             column(6, # Establish other column and repeat similar steps
                                    h2("Data Science"),
                                    plotlyOutput("dsts"),
                                    h3("Initial Thoughts"),
                                    textOutput("dstxt"),
                  ...
                  tabPanel('Forecast Data Science',
                           fluidRow(column(2), 
                  ...
                  navbarMenu("Understanding Economics",
                              tabPanel('Relationship/Predicting', # Tab 1 is relationships
                                       fluidRow(column(2), 
                                                column(8,
                                                       ...
                                                       h3(""),
                                                       textOutput(outputId = "chosetxt_fig"), # Display text
                                                       h3("")), 
                                                column(2)
                                                )
                           ),
                           tabPanel("Forecasting", # Tab 2 is forecasting
                                    fluidRow(column(2), 
                                             column(8,
...
server <- function(input, output, session) {
  ...
  shinyalert("Welcome", "Welcome to the my Economics vs. Data Science Project! This Shinyapp takes you through each stage of my process, besides the data cleaning of course :) The first tab, Exploring the Data, let's you visualize the Google Trends data for Data Science hits and Econmic hits. The second tab, Forecast Data Science, let's you follow the rational and progression for each forecasting model. Finally, the third tab, Understanding Economics, allows you to delve into the relationship between Data Science and Economics, along with predicting and forecasting Economics hits using lagged values. 
             
  Before you proceed, it is important to note that the hits number provided by Google represents the general interest of a phrase. However, it is not the raw number of total searchers, rather a sample of the total searches for that phrase."
                        )
  
  # Render Economics general time series
  output$econts <- renderPlotly({ggplot( data=df, aes(y=Economics, x=Date)) + 
  ...
  output$econtxt <- renderText({
    "This plot displays the Google Trends data for
  the phrase Economics. From this initial plot of the data, we can see that 
  people appear to be searching for Economics less overtime. There also appears
  to be some cylical trends happening, which might indicate seasonality as well.
  Now let's look at some other plots to see if this hunch is correct!"
  })
  
  # Render plot for Economics based on user choice
  output$choseplt <- renderPlotly({
    if (input$selected == "Autocorrelation") {
      autoplot(ACF(df, Economics)) + 
        ggtitle('Economics ACF Plot') +
        ylab('ACF') + 
        xlab('Lag (1 Month)') +
        theme_classic()
    } else if (input$selected == "Seasonality") {
      gg_season(df, Economics) +
  ...
  output$chosetxt <- renderText({
    if (input$selected == "Autocorrelation") {
      "From the ACF plot we can see what lags (previous values) are highly correlated with the most recent value.
      In this case, the two lags that have the highest correlation with this month's total Economics hits would be the previous
      month's number of hits and the number of hits from a year ago. We can also see that there 
      is a decreasing trend as the ACF bars are consistenlty getting smaller. 
      Furthermore, we can see some cylcial patterns in the data as the bars form wave like patterns."
    } else if (input$selected == "Seasonality") {
      "This seasonality graph indicates that there has been a general decrease in Economics hits over time. 
      The earliest years tend to be at the top of the graph, meaning that they had higher number of hits, and the more recent years
      tend to have lower number of hits. We can also see seasonal patterns in the Economics data. From January to May we see a 
      relatively constant hit count, and in June we see a drop off. Lastly we see a spike in the number of 
      hits from August to October, and after October this another decline in Economics hits. This seasonal
      component could be tied to macro-economics seasonality trends or when school is occuring. 
      Economics is a broad term so pinpointing what exactly these cycles indicate is difficult without
      other external data or analysis."
    } else if (input$selected == "Decomposition") {
      "For the Classical Decomposition model, I chose an additive model instead of multiplicative. Since the cycles appeared 
      to be somewhat consistent (no monotonic change in the variance of the cyles), I did not see a need for a Box Cox transformation or 
      multiplicative model. From the decomposition itself, we see a general decreasing trend, which is what we 
      expected from looking at the original time series and other models. As this is a classical decomposition model,
      there is no change in seasonality over time. Lastly, the random residuals look similar to white noise and cover a fairly small range. 
      This indicates that Economics hits can largely be accounted for by its trend and seasonality components."
    }
  })
  
  # Render plot for general time series for Data Science
  output$dsts <- renderPlotly({ggplot( data=df, aes(y=Data_Science, x=Date)) + 
  ...
  # Render corresponding text
  output$dstxt <- renderText({
  "This plot displays the Google Trends data for the phrase Data Science. From 
  this initial plot of the data, we can see Data Science searches are rapidly 
  increasing overtime. Furthermore, since it is a younger field, it makes sense that 
  the magnitude of its monthly hits is smaller than Economics. 
  Let's dive into some other graphs to get a better understanding of the data!"
  })
  
  # Render plot based on user choice for Data Science
  output$choseplt2 <- renderPlotly({
    if (input$selected2 == "Autocorrelation") {
      autoplot(ACF(df, Data_Science)) + 
        ggtitle('Data Science ACF Plot') +
        ylab('ACF') + 
        xlab('Lag (1 Month)') +
        theme_classic()
    } else if (input$selected2 == "Seasonality") {
      gg_season(df, Data_Science) +
  ...
  output$chosetxt2 <- renderText({
    if (input$selected2 == "Autocorrelation") {
      "From the ACF plot we can see what lags (previous values) are highly correlated with the most recent value.
      Unlike the Economics ACF plot, there is no indication of seasonality occuring as the bars are not
      going through patterns of decreasing and increasing. However, there is evidently a strong 
      trend as the bars are contiously decreasing. We can then infer that months that 
      are close to the current month, are strong indicators of what the current number of 
      Data Science hits will be."
    } else if (input$selected2 == "Seasonality") {
      "This seasonality graph indicates that there has been an increase in interest of Data Science over time.
      The earliest years tend to be on the bottom with a lower number of Data Science hits, whereas the
      more recent years are gradually higher, meaning they have more hits. Also notice how the lines
      are almost completely flat, so there are little to no seasonal componenets for Data Science. 
      Unlike the term Economics, Data Science almost exclusively refers to the field of Data Science. 
      So I would have expected to see some change tied to schools being in session. It is also possible
      ...
      "For the Classical Decomposition model, I chose an multiplicative model instead of additive. Since the cycles appeared 
      to be somewhat increasing monotonically, and there is a quick increase in the Data Science trend, a multiplicative
      model makes more sense here. As expected, there is a strong trend occuring for Data Science hits,
      as it has signifcantly increased over the last five years. There does appear to be some
      seasonality, but based on the scale and the small bar, it is fairly negligible. Lastly, our residuals look like white noise as 
      they do not have any noticable trends or cycles, however, the scale of the residuals is still fairly large.
      So, trend, more so than seasonality, makes up a decent chunk of what is occuring 
  ...
  output$chosefig <- renderPlotly({
    ...
    if (input$selected_f == "Time Series Comparison"){
      ...
      ggplot(tempdf, aes(Date, value)) +
        geom_line(aes(colour = series), size=.8) + theme_classic() +
      ...
      ggplot(df, aes(x=Data_Science, y=Economics)) + geom_point(color = 'darkred') + 
        labs(y = "Economics", x = "Data Science", 
             title = "Data Science vs Economics Hits") + theme_classic() 
    }
  ...
  output$chosetxt_fig <- renderText({
    
    if (input$selected_f == "Time Series Comparison"){
      "This plot compares the Google Trend hits of Economics and Data Science
      across the same time period. Although each line was plotted separately on the first
      tab, I thought it would be helpful to see them plotted together. As you can see,
      Economics always has more searches and generally more interests from the public.
      ...
      amount of people who go into economics in higher education. However, despite
      a relatively high number of hits, the phrase Economics has decreased over time. 
      Data Science on the other hand has started to increase over the past 5
      years. Data Science is a new field and the term is typically only used to describe
      ...
      Furthermore, Data Science and Economics have been competitive fields to a degree,
      as many PhD Economic programs are learning to applied econometrics, machine learning,
      or Data Science related topics. Since these two industries are competing and merging, 
      I thought it would be interesting to directly compare the two time series plots."
    }
    
    else if (input$selected_f == "Economics vs Data Science") {
      "Plotting two discrete variables with a wide spread over time was a challenge. 
      To best capture the data, I decided to use a scatter plot so you could see where
      each indiviudal point lied on the graph. As expected, there are a lot of high 
      Economics hits and low Data Science hits. When Data Science starts to grow 
      (has a hit value greater than 3), we can see
      that Economics stays somewhat constant. Interestingly, when Data Science
      has its highest values, we see a cluster of higher Economics searches. 
      ...
      Economics industry. So, from the time series graphs and this plot, there is perhaps a 
      relationship between Data Science and Economics, but it is difficult to tell. Let's look
      at the model we developed to see if we can predict Economics hits based 
      on Data Science hits and its own lags."
      
    } else if (input$selected_f == "Prediction Model") {
      "The purpose of this time series linear regression model was to predict
      Economics hits using only its lags and Data Science hits. Thus, the dependent 
      ...
      From the summary statistics, all three explanatory variables were statistically 
      significant, and interestingly, Data Science had a positive coefficient. More
      specefically, for each hit in the previous month's total Data Science hits,
      ...
      of the variation in Economics hits can be explained by this model. From the
      plot, which compares the actual Economics hits versus the predicted Economics hits,
      ...
      there are no extreme outliers. There is maybe some slight heteroskedasticity in the residuals, but
  ...
  output$chose_ec <- renderPlot({
    ...
    if (input$selected_ec == "Stability"){
      ...
      Economics_irf <- irf(varmodel, impulse = "Data_Science", response = "Economics", 
      ...
      plot(fc_econ, names="Economics", xlab = "Months from Start (January 2004)", ylab = "Hits", 
