...
server <- function(input, output, session) {
  ...
  output$chose_ectxt <- renderUI({
    if (input$selected_ec == "Stability") {
      p("The vector autoregression model that was developed has an
      ...
      tags$div(tags$p("Before discussing the Impulse Response Function graph, I wanted to touch on the
      ...
      br(),
      p("Now let's move onto the Impulse Response Function (IMF). IMF looks at shocks of one variable,
      ...
      p("Variance Decomposition evaluates how much the varaibles in our system,
      ...
      p("This plot depicts a vector autoregression (VAR) model based on Data Sicence and
